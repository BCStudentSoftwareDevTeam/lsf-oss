...
from app.controllers.main_routes.laborStatusForm import *
...
def test_createOverload():
    """ The above function is from the UserInsert Funtions.py and LaborStatusForm.py"""
    with app.test_request_context():
        ...
        assert checkTotalHours(termCode="202000", student="B00734292", weeklyHours="20",contractHours=None) == "20"
...
def test_approval():
    ...
    with app.test_request_context():
        ...
        with app.test_client() as c:
            ...
            adminApprovalReluctantly = c.post('/admin/modalFormUpdate', json={
            'formHistoryID': formHistory.formHistoryID, 'adminNotes': 'Approved reluctantly by me', 'status': 'Approved Reluctantly', 'formType': 'Overload'
            })

            formHistory = FormHistory.get((FormHistory.formID == lsf.laborStatusFormID) & (FormHistory.historyType == "Labor Overload Form"))
            assert formHistory.overloadForm.laborApproved.statusName == "Approved Reluctantly"
